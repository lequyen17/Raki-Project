...
from apps.deck.repositories import EDIT_ROLES
...
class CardRepository:
    ...
    def get_card_by_id(card_id, user):
        try:
            ...
            return (
                Card.objects.select_related("note", "template")
                .prefetch_related("note__values__definition")
                .filter(
                    Q(id=card_id)
                    & (
                        Q(note__deck__deck_users__user=user)
                        | Q(note__deck__share_mode="public")
                    )
                )
                .distinct()
                .get()
