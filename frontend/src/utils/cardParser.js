export const tokenizeTemplate = (templateStr, fieldValues, clozeIndex = 0, isBack = false, typedAnswers = {}) => {
  ...
  processedStr = processedStr.replace(/\{\{type:([^}]+)\}\}/g, (match, fieldName) => {
  ...
  processedStr = processedStr.replace(/\{\{c(\d+)::([^}]+)\}\}/g, (match, nStr, content) => {
